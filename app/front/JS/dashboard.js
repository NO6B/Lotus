...
async function construireCalendrier() {
    ...
    document.getElementById('cal-week-label').textContent = lundi.toLocaleDateString('fr-FR') + ' - ' + dimanche.toLocaleDateString('fr-FR');
    grid.innerHTML = ''; 

    const empty = document.createElement('div'); grid.appendChild(empty);
    const jours = [];
    for (var i = 0; i < 7; i++) {
        var d = new Date(lundi); d.setDate(d.getDate() + i); jours.push(d);
    ...
    HEURES.forEach(heure => {
        const timeLabel = document.createElement('div'); timeLabel.innerHTML = `<small>${heure}</small>`; grid.appendChild(timeLabel);
        jours.forEach(jour => {
            ...
            div.innerHTML = rdv ? 'REÇU' : '-';
            div.onclick = function() {
                const subtitle = document.getElementById('modal-subtitle');
                const actions = document.getElementById('modal-admin-actions');
                if (rdv) {
                    subtitle.innerHTML = `<p><strong>Client :</strong> ${rdv.client}</p><p><strong>Message :</strong><br>${rdv.message}</p>`;
                    actions.style.display = 'none';
                } else {
                    subtitle.innerHTML = `Créneau libre : ${dateStr} à ${heure}`;
